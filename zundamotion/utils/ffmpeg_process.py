...
async def _await_process(
    *, process: asyncio.subprocess.Process, base: str, args: List[str],
    output_path: Optional[Path], progress: _ProgressState,
    started_at: float, timeout: Optional[float], heartbeat_interval: float,
    stall_timeout: float, stdout_task: asyncio.Task[None], stderr_task: asyncio.Task[None],
) -> None:
...
async def execute_ffmpeg_process(
    args: List[str], *, base: str, output_path: Optional[Path],
    ...
    await _await_process(
        process=process, base=base, args=command, output_path=output_path,
        progress=progress, started_at=started_at, timeout=timeout,
        heartbeat_interval=heartbeat_interval, stall_timeout=stall_timeout,
        stdout_task=stdout_task, stderr_task=stderr_task,
    )
