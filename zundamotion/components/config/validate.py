...
def _validate_badge(container: Dict[str, Any], container_id: str) -> None:
    ...
    for key in ("font_color", "stroke_color"):
        ...
        if value is not None and (
            not isinstance(value, str) or not _is_valid_color_string(value)
        ):
            raise ValidationError(
    ...
    background_cfg = badge.get("background")
    if background_cfg is not None:
        if not isinstance(background_cfg, dict):
            raise ValidationError(
                f"Badge background for {container_id} must be a dictionary."
            )
        for key in ("color", "border_color"):
            value = background_cfg.get(key)
            if value is not None and (
                not isinstance(value, str) or not _is_valid_color_string(value)
            ):
                raise ValidationError(
    ...
    timing = badge.get("timing")
    if timing is not None:
        if not isinstance(timing, dict):
            raise ValidationError(
                f"Badge timing for {container_id} must be a dictionary."
            )
        start = timing.get("start", 0.0)
...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    for scene_idx, scene in enumerate(scenes):
        ...
        for line_idx, line in enumerate(lines):
            ...
            _validate_badge(line, f"scene '{scene_id}', line {line_idx}")
            line_badges = line.get("badges")
            if line_badges is not None:
                if not isinstance(line_badges, list):
                    raise ValidationError(
                        f"Badges for scene '{scene_id}', line {line_idx} must be a list."
                    )
                for badge_idx, badge in enumerate(line_badges):
                    _validate_badge_update(
                        badge,
                        f"scene '{scene_id}', line {line_idx}, badges[{badge_idx}]",
                    )
