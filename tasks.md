...
### 01. char:shake_char – 立ち絵の高速揺れ
- タイトル: `char:shake_char` – 立ち絵の高速揺れ
- 詳細: 立ち絵レイヤ overlay の `x/y` をサイン波と位相差でモジュレートし、イージングで発話時間に合わせて振幅を緩和する。
- 背景: セリフの勢いや驚きを即座に表現する頻出演出で、各シーンでの利用頻度が最も高い。
- 実装イメージ: `FilterSnippet` に `overlay_kwargs` を注入し、`resolve.py` で `amplitude/freq/easing` を統合して FFmpeg の `sin` 式に落とし込む。
