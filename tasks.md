...
* **背景**
  ...
  * 現在の動画（46秒, 29MB）は完成度が高いが、YouTube説明欄用の「チャプター」がない。
  * 視聴者にとって「何分何秒にどんなシーンが始まるか」は便利。
  * 制作者自身も動画レビューや修正点を探しやすくなる。
* **ゴール**

  * 各シーン・各セリフの開始時刻を `.csv` / `.md` で出力する。
  * YouTube説明欄にそのままコピペできる。
* **実装イメージ**

  * レンダリング時に内部で全セリフ・シーンの「開始PTS」を記録。
  * 終了後にフォーマット化して保存。
  * 出力例:

    ```
    0:00 - OP開始 (BGM: intro.wav)
    0:05 - ずんだもん「こんにちは！」
    0:12 - シーン切り替え（背景: street.png）
    ```

---

### 2. 字幕ファイル（SRT/ASS）の同時出力

* **背景**

  * 現在は字幕を焼き込み（drawtext）しているが、独立した字幕ファイルは生成されない。
  * SRT/ASSがあれば、YouTubeや翻訳に流用可能。
* **ゴール**

  * `.srt` / `.ass` 字幕ファイルを動画と同時に出力する。
* **実装イメージ**

  * 内部で生成しているセリフ情報からタイムスタンプを抽出。
  * Pythonの `pysubs2` 等でSRT/ASSに変換。
  * 出力例（SRT）:
