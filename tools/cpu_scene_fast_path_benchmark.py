...
The benchmark intentionally uses no persistent cache, a fixed YAML scenario, the
locked runtime, and alternating standard/fast trials. It records wall time,
PerfSummary fields, path-selection evidence, and decoded media hashes.
"""
...
def run_benchmark(args: argparse.Namespace) -> dict[str, Any]:
    ...
    path_selection_verified = all(
        bool(item["path_selection_verified"]) for item in trials
    )
    ...
    meaningful_improvement = bool(
        elapsed_ratio is not None
        and elapsed_ratio <= 0.90
        and video_phase_ratio is not None
        and video_phase_ratio <= 0.90
    )
...
def main() -> int:
    ...
    output_path = args.output_dir.resolve() / "cpu-scene-fast-path-benchmark.json"
    print(output_path)
    comparison = result["comparison"]
    # Benchmark execution fails only when the selected paths or media semantics are invalid.
    # A slower valid experiment is a successful measurement with decision=keep_standard.
