...
| subtitle gap copy 最大化 | 却下 | `000_intro_channel-intro` では `SubtitleGap count=0`。削れる gap がない |
| subtitle PNG tight bbox 化 | 却下 | PNG は字幕ボックス相当サイズ。crop は背景ボックス表現を壊すリスクが高い |
| subtitle PNG input 共有 | 却下 | `000_intro_channel-intro` では duplicate total `0`。input count は減らない |
| face overlay input 共有 | 却下 | `000_intro_channel-intro` では duplicate total `0`。input count は減らない |
