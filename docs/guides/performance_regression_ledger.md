...
## 2026-07-17 scene fingerprint正当性と旧cache読込撤去

何をしたのか:

- scene keyへ、`defaults.characters`、キャラクター標準scale/anchor、実効characters/background persist、状態解決versionを追加した。
- 解決済み`asset_name`、flip、正規化color filter、z、画像パスをキャラクターfingerprintへ統一し、画像差替えは既存のsize+SHA-256署名で判定する。
- move、enter、leave、動的scale/position/effectを持つキャラクターは静的scene baseへ焼き込まない。
- 旧scene/subtitle cacheのフォールバック読込とlegacyログを削除し、古いファイルは走査・変換・削除せず無視する。
- base keyから字幕専用の`subtitle`、`subtitle_text`、`subtitle_config`だけを除外し、表示字幕だけの変更ではbaseを再利用する。

性能上の判断:

- 描画結果が変わる設定・素材変更では意図的にMISSが増えるが、誤HITによる古い映像再利用を防ぐ正当性を優先する。
- 字幕表示だけの変更ではbase HITを維持し、subtitle-burned sceneだけを再生成する。
- color filter付きキャラクターは正確性優先で行単位描画を維持する。静的base最適化はfiltered PNGの解決を共通化できた時点で再検討する。
- Python 3.14の回帰テストで同一入力HIT、既定値・persist・asset・flip・color・同一パス画像差替えMISSを確認した。4回の実動画比較は未実施。
