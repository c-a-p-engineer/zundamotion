...
- YAML、CLI、環境変数、テンプレートに設定項目を追加・変更した場合は、利用者向けドキュメントの正本も同じ変更で更新する
- 各設定項目について「項目名」「意味」「設定可能値」「デフォルト値」を必ず記載する
- 未指定を許す場合は省略時挙動を記載し、YAML・CLI・環境変数・プリセット間に優先順位がある場合はその順序も記載する
- 内部実装だけが使う値と利用者が指定できる値を区別し、内部対応値をそのまま公開設定の選択肢として記載しない
- 同じ説明を複数箇所へ複製せず、`scripts/script_cheatsheet.md` などの正本へ詳細を置き、機能一覧やREADMEからは必要に応じて誘導する
