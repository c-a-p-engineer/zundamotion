...
| [`sample_overlay_blink.yaml`](../scripts/sample_overlay_blink.yaml) | 静止画像 overlay blink スモーク | `blink` 単体、`opacity` 併用、速い/遅い点滅、`shake` 併用を確認 |
| [`benchmark_perf_summary.yaml`](../scripts/benchmark_perf_summary.yaml) | Performance summary スモーク | FFmpeg/ffprobe/cache/字幕PNG/中間ファイル集計を短時間で確認 |
| [`benchmark_voice_cache_reuse.yaml`](../scripts/benchmark_voice_cache_reuse.yaml) | Voice cache reuse ベンチ | 同一テキスト音声の content-addressed cache 再利用を確認 |
...
| [`sample_character_move.yaml`](../scripts/sample_character_move.yaml) | 立ち絵の座標移動 | `move.from` から `position` への補間、`characters_persist` による前位置参照、揺れとの併用 |
...
| [字幕設定](../scripts/script_cheatsheet.md#字幕設定) | [`sample.yaml`](../scripts/sample.yaml), [`sample_subtitle_styles.yaml`](../scripts/sample_subtitle_styles.yaml), [`sample_subtitle_render_modes.yaml`](../scripts/sample_subtitle_render_modes.yaml) |
| [キャラクター表示](../scripts/script_cheatsheet.md#キャラクター表示) | [`sample_character_enter.yaml`](../scripts/sample_character_enter.yaml), [`sample_character_move.yaml`](../scripts/sample_character_move.yaml), [`sample_character_color_filter.yaml`](../scripts/sample_character_color_filter.yaml), [`copetan_all_expressions.yaml`](../scripts/copetan_all_expressions.yaml), [`engy_all_expressions.yaml`](../scripts/engy_all_expressions.yaml) |
