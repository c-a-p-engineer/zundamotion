...
- `j_cut.duration` の現在の互換挙動は、指定行の尺先頭へ pre-roll を追加し、その同じ秒数だけ指定行の音声開始を遅らせます。字幕は pre-roll の先頭から表示されます。一般的な「次の音声が先に聞こえ、映像切替が後から来る」J-cut semantics ではありません。真のJ-cutとしての重なりは未実装です。
