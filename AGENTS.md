...
機能を削除したり、不採用判断で実装を落とした場合でも、その判断理由と再検討条件は `docs/issues_pending.md` または `docs/guides/` に残し、ドキュメント上の意思決定履歴を消さないこと。
...
- 仕様の認識に齟齬がある可能性がある場合は、実装前にユーザーへ確認する。設定ミスの補正か仕様変更かが曖昧な挙動、特に `visible` 未指定時の表示や暗黙的なキャラクター補完は、確認なしに変更しない。
