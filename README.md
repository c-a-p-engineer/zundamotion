...
- **字幕焼き込みの ASS 優先化**: 完成版の字幕合成は既定で `ASS/libass` を使い、従来の PNG 連続 overlay より最終段のコストを下げます。必要なら `subtitle.render_mode: png` で戻せます。
...
より詳しい指針や貼り方のコツは [AI_README.md](/docs/AI_README.md) を参照してください。
...
- GPUオーバーレイ方針: 完成版の字幕焼き込みは既定で `ASS/libass` を使います。PNG 合成が必要なケースだけ `subtitle.render_mode: png` に戻してください。RGBAを含む overlay は引き続きCPU側で合成し、RGBAを含まない場合はGPU（overlay_cuda/scale_cuda or scale_npp）を利用します。
...
- ハイブリッドGPUスケール: RGBAオーバーレイでCPU合成となる場合でも、背景のスケーリングのみGPUで実施してからCPUへ戻す最適化が可能です（`video.gpu_scale_with_cpu_overlay: true` 既定有効）。CUDA が使えない環境では OpenCL によるスケール専用（`scale_opencl`）をスモークに通った場合に限り自動で利用します。
 ...
 - 字幕焼き込みモード: `subtitle.render_mode: ass` が既定です。完成版の字幕焼き込みを 1 フィルタへ寄せられるため、最終段の再エンコード負荷を下げられます。
