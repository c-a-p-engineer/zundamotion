...
- **テキストバッジ表示**: `badge` を指定すると、「重要」「注意」などの短いラベルを角丸バッジとして動画上部へ表示できます。
...
短い注目ラベルを出したい場合は `badge` / `badges` を指定します。省略時は何も表示されません。共有して再利用したい持続バッジは `scenes` の外に top-level `badges` として置けます。
    ...
    lines:
      - id: intro
        ...
        badges:
          - id: important-top
            visible: true
      - text: "この行だけ複数バッジにする例です。"
        badges:
          - text: "長い文でも自動で幅が伸びる注意バッジ"
            position: "top-left"
            timing: {start: 0.4, end: 1.4}
          - text: "補足"
            position: "top-right"
...
- `text`: 必須。表示する短い文字列です。
- `position`: 必須。`top-left` / `top-center` / `top-right` / `bottom-left` / `bottom-center` / `bottom-right` を指定します。
- `badges` / `scene.badges` / `line.badges`: 任意。複数バッジを扱う基本形です。top-level `badges` は全 scene で再利用できる共有定義、`scene.badges` はその scene 専用の定義または共有定義の scene 上書きです。`line.badges` には `{id: "...", visible: true/false}` の更新と、その行だけの直接表示バッジの両方を書けます。
- `font_size` / `font_color` / `stroke_color` / `stroke_width` / `background`: 任意。字幕の見た目に近い指定ができます。バッジ幅は文字数に応じて自動で伸び、`min_width` / `max_width` で制御できます。
- `timing.start` / `timing.end`: 任意。表示開始秒と表示終了秒です。`end` を省略すると、そのシーンまたは行の終わりまで表示します。
- `timing.show_on_line` / `timing.hide_on_line`: 任意。scene-level バッジを特定発話の開始時点で表示・非表示にします。値は 1-based 行番号または `line.id` です。
- 既存互換として単体の `badge` も使えますが、新規では `badges` を推奨します。
- バッジは内部で固定サイズの PNG として一度だけ生成・キャッシュし、既存の前景オーバーレイ経路で合成します。`badge` を使わない既存動画の挙動は変わりません。
