...
- 公式推奨・動作保証対象は最新安定版の CPython 3.14 系です。パッチ版は固定しません。
- Python 3.10 以上 3.13 以下でもインストールと実行は可能ですが、互換性テスト、継続的な互換性維持、個別の不具合修正は保証しません。Docker GPU イメージはベースOSの都合で Python 3.10 を使います。
- alpha / beta / RC / dev 版は公式サポート対象外です。
