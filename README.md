...
- 公式サポート対象は最新安定版の CPython 3.14 系です。パッチ版は固定しません。
...
Dev Container は CPU が標準です。GPU を使う場合は
`.devcontainer/gpu/devcontainer.json` を選択します。この定義は GPU 用 Compose
override で `Dockerfile.gpu` を明示するため、`.env` の `ZUNDA_DOCKER` を切り替える
必要はありません。ビルド済みイメージの構成は
`/opt/zundamotion-build-info/build-info.json`、定義の固定値は
`python scripts/check_runtime_versions.py` で確認できます。
